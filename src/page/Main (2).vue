    ...
    <el-table :data="pagedData" @row-click="handleRowClick" highlight-current-row>
      ...
      <el-table-column label="操作">
        <template #default="{ row }">
          ...
          <el-button @click="deleteItem(row)" type="danger">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  computed: {
    pagedData() {
      ...
      return paged.map((item, index) => ({ ...item, index: start + index + 1 }));
  ...
  methods: {
    ...
    editItem() {
      // 编辑事件处理逻辑
      // if (!this.selectedRow) {
      //   console.log('请先选择需要编辑的列表项');
      // } else {
      //   console.log('编辑', this.selectedRow);
      //   const selectedRowId = this.selectedRow.ProcessingRecord;
      //   const detailApiUrl = `/api/detail/${selectedRowId}`;

      //   // 跳转到编辑页面并传递 ID 参数
      //   // this.$router.push({ name: 'Detail', params: { id: selectedRowId } });
    ...
    deleteItem() {
      // 删除事件处理逻辑
      if (!this.selectedRow) {
        console.log('请先选择需要删除的列表项');
      } else {
        console.log('删除', this.selectedRow);
      }
    },
    handleRowClick(row) {
